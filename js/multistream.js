var opts = {
	...
	opacityFlowFocusDeselected : 0.30, //
	opacityTextLabel : 1,
	...
	pathCandadoOpen: ".\\img\\lockpad_open.png",
	pathCandadoClose: ".\\img\\lockpad_close.png",
	//
	//
	//
...
var ranges = []; // Comming from json file
var type; //Comming from json file, data type
var y_axis_label_focus; //label Y focus
var rangesDomainFocus; 
var svg; 
var dataset;
var tooltip;

var nest = d3.nest().key(function(d) {return d.key}); //Group by key
...
var scalesFocus = []; 
var axisFocus = [];
function addFeatures(element) {
	//index 0 parce tous sont les memes
	element.name = element.values[0].name;
	element.level = element.values[0].level;
	element.parentKey = element.values[0].parentKey;
	element.linkedTo = element.values[0].linkedTo;
	element.from = element.values[0].from;
	element.end = element.values[0].end;
	element.color = element.values[0].color;
};			
...
var valueline = d3.svg.line()
					...
					;
//					.y(function(d) {return yScaleFocus(d.y0 + d.y);});
...
var dataPreProcessing1 //datasource
var dataPreProcessing2 //datasource

var dataPreRangeLevel1; //Currently data Context
var dataPreRangeLevel2; //Currently data Focus
...
function changeGranularity(){
	...
	dataPreProcessing1 = preProcessing();// To use later
	dataPreProcessing2 = preProcessing(); //To use later
	
	filteredRangesInContext=Array.from(dataPreProcessing1);
	filteredRangesInFocus =Array.from(dataPreProcessing2);
	updateFlows();
}

function changeOffset(){
	if(filteredRangesInContext==null){
		filteredRangesInContext=Array.from(dataPreProcessing1);
	}
	if(filteredRangesInFocus==null){
		filteredRangesInFocus =Array.from(dataPreProcessing2);
	}
	updateFlows();
}
...
function filterFlow() {
	var	cloneContext = Array.from(dataPreProcessing1);
	var	cloneFocus = Array.from(dataPreProcessing2);

		filteredRangesInContext = cloneContext.filter(function(element){
			if(checkAvailability(rangeVisualize,element.key)){
				return element;
			}
		})
		filteredRangesInFocus = cloneFocus.filter(function(element){
			if(checkAvailability(rangeVisualize,element.parentKey)){
				return element;
			}
		})
	updateFlows();
...
function updateFlows(){
	...
	dataCurrentlyContext = stack(nest.entries(filteredRangesInContext));
	dataCurrentlyContext.map(addFeatures);
	
	//Update Axis
	var max = d3.max(dataCurrentlyContext, function(aray){
	...
	fContext.transition()
			.duration(opts.durationTransition)
				...
				.style({
//						"fill" : function(d) {return GetRGBColorString((d.from + d.end)/2);},
//						"stroke" : function(d) {return GetRGBColorString((d.from + d.end)/2);},
...
//					"stroke" : function(d) {return GetRGBColorString((d.from + d.end)/2);},
	...
	//--FOCUS---
	dataCurrentlyFocus = stack(nest.entries(filteredRangesInFocus));//Used in Focus Local
	dataCurrentlyFocus.map(addFeatures);
	...
	yScaleFocus.domain(yScaleContext.domain()); //REPAIR
	...
	flowFocusNormal.transition(t)
				.duration(opts.durationTransition)
					...
					.style({
//							"fill-opacity" : function(d) {return opts.opacityFillFocusNormal;},
//							"stroke-opacity" : function(d) {return opts.opacityFlowFocusStroke;},
							"fill" : function(d) {return d.color},
							"stroke" : function(d) {return d.color}
					});
...
$(document).ready(function() {
	...
	d3.select("#time").on("mouseup", function() {
		...
		changeGranularity();
		//nTimeGranularity = +this.value;
		//getDataProcessing(+this.value);
		//updateTransitionFocus();
	});
	...
	ranges.forEach(function(element){
		d3.select("#"+element.key).on("click", function() {
				document.getElementById(element.key).classList.toggle("btn-primary");
				var empty = true;
				ranges.forEach(function(element){
				...
				if(empty){
					document.getElementById(element.key).classList.add("btn-primary")
				}
				createBtnHierachy();
				filterFlow()
		});
...
function initialization() {
	...
	dataset = jsonArray.data.map(function(d, i) {
		if(getText){ //Variable in visualize.php to get or not the text
			return [ parseDate(d.date_time), d.value, d.text ];
		}else{
			return [ parseDate(d.date_time), d.value];
		}
	});
	ranges = jsonArray.ranges;// jsonArray from PHP file
	
	(jsonArray.type!=null) ? type = jsonArray.type : type = "values"; //data type
	(jsonArray.type!=null) ? y_axis_label_focus = jsonArray.type : y_axis_label_focus = "values"; //data type
	
	//ADDING features to ranges like : name, key, parent, level
	ranges.forEach(function(element, index){
		element.key = "R"+index;
		element.parentKey = "R"+index;
		element.level = 1;

		var numberChild = element.children.length;
		var color_root;
		ranges.length <	 7 ? color_root = colores_d3(index) : color_root = colores_brewer(index);	
		
		element.color =chroma(color_root).desaturate().brighten(0.4);

		var color_begin_interpo = "white";
		var color_finish_interpo = chroma(color_root).saturate().darken();

		//var color_finish_interpo = chroma(color_root).darken();
		//rangeColorHijo =  chroma.scale(["white",chroma(colorPadre(index)).darken()]).colors(numberChild+1);
		//color linear interpolation *****
		rangeColorHijo =  chroma.scale([color_begin_interpo,color_finish_interpo]).colors(numberChild+1);
		//color bezier interpolation ***
//		rangeColorHijo =  chroma.bezier([color_begin_interpo,color_finish_interpo]).scale().colors(numberChild+1);
		
		element.children.forEach(function(elementChildren, indexChildren){
			elementChildren.key="R"+index+"_"+indexChildren;
			elementChildren.parentKey = "R"+index;
			elementChildren.level = 2;
//			elementChildren.color = element.color; // Pone el mismo color para parecer que no hay rangos;
			elementChildren.color = rangeColorHijo[++indexChildren];
		})
	})	
	
	createHierarchyButtons();
	createSvg(); //create svg main and axes
	display(); //jsonArray.ranges is from .json file
}

function createHierarchyButtons(){
	ranges.forEach(function(element){
		d3.select("#col-hierarchy").append("button")
						.attr("id",element.key)
						.attr("type","button")
						...
						.attr("data-original-title",element.name)
						.style({
								"width":"auto",
								"margin-left":"2px",
								"margin-right":"2px",
						...
						.on("mouseover",function() {
								var selectedFlow = this; //get d selectione for use later in transition
							
								focus.select("#flowsInFocus").selectAll(".focus")
									.transition()
									.duration(opts.tooltipTransitionMouseMove)
										.style({
											"opacity":function(d){return (selectedFlow.id == d.key || selectedFlow.id == d.parentKey ) ? opts.opacityFlowFocusSelected : opts.opacityFlowFocusDeselected;},
											"stroke-width":function(d){return (selectedFlow.id == d.key || selectedFlow.id == d.parentKey ) ? opts.strokeWidth : 0;}
										});
										
								focus.select("#textsLabels").selectAll(".textLabel")
											.style({
												"fill-opacity":function(d){return (selectedFlow.id == d.key || selectedFlow.id == d.parentKey ) ? opts.opacityTextLabelSelected : opts.opacityTextLabelDeselected;},
												"font-weight":function(d){return (selectedFlow.id == d.key || selectedFlow.id == d.parentKey ) ? "bold" : "normal";}
											});
						})
...
function display() {
	...
	dateExtRange = d3.extent(dataset, function(d) {return d[0];}); // max and min date
	...
	// preProcessing is a function to divide a data by range de time and range of R0, R1, R3, R4
	var justOnePreprocessing = preProcessing();

	
	var start =  new Date().toLocaleString()
	dataPreProcessing1 = justOnePreprocessing.filter(function(element){
		return element.level == 1;
	});
	
	var dataPreRangeLevel1 = Array.from(dataPreProcessing1); 
	dataRangeLevel1 = stack(nest.entries(dataPreRangeLevel1));//Used in Focus Local
	dataRangeLevel1.map(addFeatures); 
	
	dataPreProcessing2 = justOnePreprocessing.filter(function(element){
		return element.level == 2;
	});
	
	var dataPreRangeLevel2 = Array.from(dataPreProcessing2); //	dataPreRangeLevel2 = JSON.parse(JSON.stringify(dataPreProcessing2));
	dataRangeLevel2 = stack(nest.entries(dataPreRangeLevel2));//Used in Focus Dist and Detail
	dataRangeLevel2.map(addFeatures);  

	/* DOMAIN IN FOCUS AND CONTEXT*/
	
	xScaleContext.domain(dateExtRange);
	
	yScaleFocus.domain([ 0, d3.max(dataPreRangeLevel1, function(d) {return d.y0 + d.y;}) ]);
...
//	var intervalTime = nTimeGranularity * 10;
	switch (timePolarity) {
		case 0: 
//			xAxisContext.ticks(d3.time.minute, 1).tickFormat(customTimeFormat);
//			xAxisContext.tickSize(-heightContext)
			xAxisContext.ticks(20).tickFormat(customTimeFormat)
			xScaleContext.nice(d3.time.minute);
			break;
		case 1: 
//			xAxisContext.ticks(d3.time.hour, intervalTime).tickFormat(customTimeFormat);
			xAxisContext.ticks(10).tickFormat(customTimeFormat);
			xScaleContext.nice(d3.time.hour);
			break;
		case 2: 
//			xAxisContext.ticks(d3.time.day, intervalTime).tickFormat(customTimeFormat);
			xAxisContext.ticks(10).tickFormat(customTimeFormat);
			xScaleContext.nice(d3.time.day);
			break;
		case 3: 
//			xAxisContext.ticks(d3.time.week, intervalTime).tickFormat(customTimeFormat);
			xAxisContext.ticks(10).tickFormat(customTimeFormat);
			xScaleContext.nice(d3.time.week);
			break;
		case 4: 
//			xAxisContext.ticks(d3.time.month, intervalTime).tickFormat(customTimeFormat);
			xAxisContext.ticks(10).tickFormat(customTimeFormat);
			xScaleContext.nice(d3.time.month);
			break;
		case 5: 
//			xAxisContext.ticks(d3.time.year, intervalTime).tickFormat(customTimeFormat);
			xAxisContext.ticks(10).tickFormat(customTimeFormat);
	...
	flowContext = context.select("#flowsInContext").selectAll(".area") //just for 4 flow, 1 level
									.data(dataRangeLevel1,function(d) { return d.key; });
	...
	//To get the ranges for each scale in focus
	rangesDomainFocus = (nest.entries(calculateRangeFocus(opts.facteurNor, opts.facteurDis, opts.facteurZoom)));
	...
	createGradientArrays();
	//createPatternFondArrays();
	
	//FOCUS
	
	focus.append("g").attr("id","flowsInFocus");
	
	focus.select("#flowsInFocus").selectAll(".focus.area0")
					.data(dataRangeLevel1,function(d) { return d.key; })
					.enter()
					.append("path")
					.attr("id", function(d){return "focus_area0_" + d.key})
...
//					.attr("data-target", "#infoModal")
	...
	for(var index=1;index<4;index++){
		focus.select("#flowsInFocus").selectAll(".focus.area" + index)
					.data(dataRangeLevel2,function(d) { return d.key; })
					.enter()
					.append("path")
					.attr("id", function(d){return "focus_area"+index+"_" + d.key})
					.attr("class", function(d){return "focus area" +index +" "+  d.parentKey})
...
//	d3.select(this).attr("data-target", "#dataModal");
	//					d3.select(this)..attr("data-toggle","tooltip")
//					d3.select(this)..attr("title","kaka")
//					d3.select(this)..attr("data-original-title","pedo")
//	document.getElementById("erick").setAttribute("data-toggle","ddd")
//	document.getElementById("erick").setAttribute("title","ddd")
//	document.getElementById("erick").setAttribute("data-original-title","ddd")
...
function getTextLabels(){
	...
	if(dataCurrentlyFocus==null){
		dataInZoomArea = dataRangeLevel2;
	}else{
...
//				console.log(element.name + " and " + elementOther.name + " : " + intersectRect(rectA, rectB))			
...
function createTooltip(){
	...
	var msgDataModal;
	var titleDataModal;
	var textSelected; // verifie if text was load in the init() function
	
	focus.select("#flowsInFocus")
			.selectAll(".focus")
			.on("mouseover",function(d, i) {
					...
					focus.select("#flowsInFocus").selectAll(".focus")
						...
						.duration(opts.tooltipTransitionMouseMove)
							.style({
									"stroke":function(d){
										var new_color = chroma(d.color).darken();
										//just for the level 2 paths
										return (selectedFlow.key == d.key && 2 == d.level ) ? new_color: 0;
									},	
									"opacity":function(d){
										return (selectedFlow.key == d.key || selectedFlow.key == d.parentKey ) ? opts.opacityFlowFocusSelected : opts.opacityFlowFocusDeselected;
									},
									"stroke-width":function(d){
										return (selectedFlow.key == d.key || selectedFlow.key == d.parentKey ) ? opts.strokeWidth : 0;
									},
								})		
					...
					focus.select("#textsLabels").selectAll(".textLabel")
								.style({
									"fill-opacity":function(d){
										return (selectedFlow.key == d.key || selectedFlow.key == d.parentKey ) ? opts.opacityTextLabelSelected : opts.opacityTextLabelDeselected;
									},
									"font-weight":function(d){
										return (selectedFlow.key == d.key || selectedFlow.key == d.parentKey ) ? "bold" : "normal";
									},
								});
//					focus.selectAll(".x.grid")
//								.style({
//									"stroke-opacity":0.05}
//								);					
...
//					var i = bisectDate(points, mousex);
					...
					var dateReformated = d.values.map(function(element) {return new Date(element.date)});
					var mouseDateIndex = dateReformated.map(Number).indexOf(+mouseSelectedDate); //To get the index of the date in array
...
//								.style("opacity", 1);
						...
						textSelected = (d.values[mouseDateIndex].text); //ARRAY
						var formatDate;
						...
						tooltip
							...
							.duration(opts.tooltipTransitionMouseMove)
								...
								.style("background", "rgba(255, 255, 255, 0.75)")
									var first_line = "<p class='title' style='opacity: 1;'>" + d.name + "</p>";
									var second_line = "<p class='info'> At " + formatDate(dateSelected) + " " + customTimeFormat(dateSelected) + ", " + "<strong>" + valueSelected +" "+  type + "</strong></p>";
									var thrid_line = "";
									
									if(textSelected.length!=0){
										thrid_line = "<p class='message'><strong>Click</strong> to see the " + type +  "</p>";
										
										msgDataModal = "";
										titleDataModal = "";
										titleDataModal = "<strong><p style='text-transform: capitalize;'>" + d.name + "</p></strong>"  + second_line;
										msgDataModal = "<table class='table' style=width:'100%'>";
										for(var i = 0; i<textSelected.length;i++){
											msgDataModal = msgDataModal + "<tr><td>" + textSelected[i] + "</td></tr>";
										}
										msgDataModal = msgDataModal + "</table>";
									}
									
	//								var thrid_line = "<p class='text'>" + textValue + "</p>";
									//(isFilter) ? thrid_line= "<p class='info'>Click to back" + "</p>" : thrid_line= "<p class='info'>Click to filter this category" + "</p>";
									
//									first_line = "<div class='alert alert-info alert-dismissable fade in'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>Info!</strong> This alert box could indicate a neutral informative change or action.</div>";
			...
			.on("mouseout", function(d, i) {
					...
					focus.select("#flowsInFocus").selectAll(".focus")
								.transition()
										.style({"opacity":1,"stroke-width":opts.strokeWidth})
...
function createGradientArrays() {
	...
	var newGradientLeftStroke = function(index, colorBegin, colorEnd) {
		...
		gradientFocusLeftStroke.push({
			cle : index
		});
	}

	ranges.forEach(function (element, index){
		element.children.forEach(function (elementChild,indexChild){
//			var colorBegin = GetRGBColorString((element.from + element.end) / 2);
//			var colorEnd = GetRGBColorString((elementChild.from + elementChild.end) / 2);
			
			var colorBegin =  element.color; // GetRGBColorString(66);
			var colorEnd = elementChild.color; // "red"; // color(index);

			newGradientLeft(elementChild.key, colorBegin, colorEnd); // Gradient Left
			newGradientLeftStroke(elementChild.key, colorBegin, colorEnd); // Gradient Left Stroke

			newGradientRight(elementChild.key, colorEnd, colorBegin); // Gradient Right
			newGradientRightStroke(elementChild.key, colorEnd, colorBegin); // Gradient Left Stroke
		})
	})
}
...
//	console.log("iNorLeft " + iNorLeft)
//	console.log(iFunctionL)
//	console.log(sumDisLeft)
//	console.log(sDisLeft)
...
function updateFocus() {
	...
	if (calcule != null) {
		...
		rangesDomainFocus = (nest.entries(calcule));
...
/**
 * 
 * Function to divide dataset by range of DATES and range R0 R1 R2 R3 ...
 * 
 * @param dataset
 *            dataset bruto with dates and values
 * @param range
 *            number of ranges to divide
 * @returns {Array}
 */
function preProcessing() {
	...
	for (var i = 0; i < dateRange.length; i++) {
		var dateRangeBegin = dateRange[i]; 
		var dateRangeEnd = dateRange[(i + 1)];
		var dateR = dataset.filter(function(element) {
			// Calcule verified : 15/04/2016, 
			// For example FROM 17:39:00 TO 17:39:59 and continue A 17:40:00.....
			return element[0] >= dateRangeBegin && element[0] < dateRangeEnd;
		});

		// Loop from array of ranges R0, R1, R2, R3... and from , to
		for(var j=0;j< ranges.length;j++){
			var valueRangeParent = 0;
			var textFather = [];
			for(var k=0;k<ranges[j].children.length;k++){
				var total = [];
				var valueRange = dateR.filter(function(element) {
					//to compare en lowercase..
					return element[1].toLowerCase() == ranges[j].children[k].name.toLowerCase();
				});

				var text = [];
				valueRange.forEach(function(elementText){
					if(elementText[2]!=null){//text
						text.push(elementText[2])
						textFather.push(elementText[2])
					}
				})
				
				total = total.concat(valueRange); // Concatening 2 arrays to get the length
				valueRangeParent = valueRangeParent + total.length;
				results.push({
					"date" : dateRangeBegin,
					"key" : ranges[j].children[k].key,
					"value" : total.length,
					"parentKey" : ranges[j].children[k].parentKey,
					"name":ranges[j].children[k].name,
					"level":ranges[j].children[k].level,
					"linkedTo":ranges[j].children[k].key,
					"from":ranges[j].children[k].from,//used to color
					"end":ranges[j].children[k].end,//used to color
					"color":ranges[j].children[k].color, //d3.hsl(h, s, l)
//					"colorStroke":ranges[j].children[k].colorStroke, //d3.hsl(h, s, l)
					"text":text
				});
			}
			results.push({
				"date" : dateRangeBegin,
				"key" : ranges[j].key,
				"value" : valueRangeParent,
				"parentKey" : ranges[j].key,
				"name":ranges[j].name,
				"level":ranges[j].level,
				"linkedTo":ranges[j].children,
				"from":ranges[j].from, //used to color
				"end":ranges[j].end, //used to color
				"color":ranges[j].color,
//				"colorStroke":ranges[j].colorStroke,
				"text":textFather
			});
		}
	}
 ...
 * @returns
 */
//function createStack(dataset) {
////	console.log("offsetType : " + offsetType)
//
////	stacki(nest.entries(dataset));
//	
////	var jj =  layer[layer.length-1];
////	var reformated = layer[layer.length-1].values.map(function(obj){return obj.y + obj.y0}); 
////	console.log(d3.max(reformated));
////	console.log(layer);
//	
////	console.log(layer)
//	return stack(nest.entries(dataset));
//}
